...
import { type BankDetails, BankDetailsList, catalogApi, type ContactPerson, type Supplier } from '../../features/catalog';
...
export default function SuppliersPage() {
  ...
  const { canEdit, canDelete } = useModuleAccess('catalog.suppliers');
  ...
  const { data: suppliersData, isLoading } = useQuery({
    queryKey: ['suppliers', searchText],
    queryFn: () => catalogApi.suppliers.list({ search: searchText || undefined }),
    staleTime: 0,
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Partial<Supplier>) => {
      ...
      if (editingSupplier) {
        supplier = await catalogApi.suppliers.update(editingSupplier.id, data);
        
        // Определяем какие контакты удалить, обновить или создать
        const existingIds = new Set(editingSupplier.contacts?.map(c => c.id) || []);
        const currentIds = new Set(contacts.filter(c => c.id).map(c => c.id!));
        
        // Удаляем контакты, которых нет в текущем списке
        for (const contact of editingSupplier.contacts || []) {
        ...
        supplier = await catalogApi.suppliers.create(data);
    ...
    onSuccess: () => {
      message.success(editingSupplier ? 'Поставщик обновлён' : 'Поставщик создан');
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => catalogApi.suppliers.delete(id),
    onSuccess: () => {
  ...
  const handleEdit = async (supplier: Supplier) => {
    ...
    try {
      const fullData = await catalogApi.suppliers.get(supplier.id);
      ...
      const bankDetailsData = await catalogApi.bankDetails.bySupplier(supplier.id);
      setBankDetails(bankDetailsData);
      
      setModalOpen(true);
    } catch (error) {
      message.error('Ошибка загрузки данных поставщика');
    }
  };
  ...
  const handleDeleteContact = (index: number) => {
    ...
    // Если удалили основной, назначаем первого
    ...
    <div className="page-container">
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <Text type="secondary">Справочник поставщиков материалов и комплектующих</Text>
        </div>
        {canEdit && (
          <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>Добавить поставщика</Button>
        )}
      </div>
      ...
      <Card size="small">
        {isLoading ? (
          ...
          <Empty description="Нет поставщиков" />
        ) : (
          <Table
      ...
      <Modal
        title={editingSupplier ? 'Редактирование поставщика' : 'Новый поставщик'}
        open={modalOpen}
        ...
        <Tabs defaultActiveKey="info" items={[
            ...
            children: (
              <Form form={form} layout="vertical">
                <Form.Item name="name" label="Краткое наименование" rules={[{ required: true, message: 'Введите наименование' }]}>
                  <Input placeholder="Например: ООО «Поставщик»" />
                </Form.Item>
          ...
          },
          {
            ...
            children: editingSupplier ? (
              ...
              <Empty 
                description="Сохраните поставщика, чтобы добавить банковские реквизиты" 
                style={{ margin: '40px 0' }}
              />
            ),
          },
